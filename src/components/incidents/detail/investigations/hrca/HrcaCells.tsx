...
export function HrcaCategoryCell(
  ...
  return (
    <div className="flex h-full min-h-[128px] items-center justify-center self-stretch rounded-[9px] border border-[rgba(15,23,42,0.08)] bg-[rgba(8,145,166,0.13)] px-[11px] py-[9px]">
      <p className="text-ehs-dark-bg text-center text-[12px] leading-[14.4px] font-bold">
        {lines.map((line) => (
...
export function HrcaContributingFactorCell(
  ...
  return (
    <button type="button" onClick={onEdit} className={[cardShell, "cursor-pointer hover:border-[rgba(11,19,32,0.22)]"].join(" ")}>
      <p
        className="mb-1.5 text-[9px] leading-[13px] font-bold tracking-[0.72px] uppercase"
        style={{ color: accent }}
      >
        Contributing factor
      </p>
      <p className="text-ehs-dark-bg text-[12.5px] leading-[18.13px] font-bold">
        {text}
      </p>
    </button>
  );
      ...
      <div className="relative h-full min-h-[128px]">
        ...
        >
          <div className="mb-[7px] flex w-full items-center gap-[7px]">
            ...
            {why.isRootCause ? (
              <span
                className="inline-flex items-center gap-[3px] text-[9px] font-bold tracking-[0.54px] uppercase"
                style={{ color: accent }}
              >
              ...
              <Text
                as="span"
                className="text-ehs-muted-text text-[9px] font-bold tracking-[0.54px] uppercase"
              >
                {`Why ${why.num}`}
              </Text>
            )}
            <span
              ...
              className="text-ehs-muted-text ml-auto inline-flex size-[18px] items-center justify-center rounded-[5px] opacity-40 transition-opacity hover:opacity-100 hover:text-[#e11d48]"
              aria-label="Remove why step"
            >
              <Icon icon="mdi:close" className="size-[11px]" aria-hidden="true" />
            </span>
          </div>
          <p className="text-[12.5px] leading-[18.13px] font-normal text-[#2a3446]">
            {why.text}
          </p>
        </button>
      </div>
    ...
    return (
      <div className="relative h-full min-h-[128px]">
        ...
        >
          ...
          <Text as="span" className="text-ehs-muted-text text-[10.8px]">
            Add why
          </Text>
        </button>
      </div>
  ...
  return (
    <div className="relative h-full min-h-[128px]">
      ...
      <div className="flex h-full min-h-[128px] w-full items-center justify-center rounded-[12px] border border-dashed border-[rgba(15,23,42,0.14)] px-[14px] py-[13px]">
        <Text as="span" className="text-[11px] leading-[15.95px] text-[#b3bbc8]">
          —
        </Text>
      </div>
    </div>
...
export function HrcaCorrectiveActionsCell(
  ...
  return (
    <div className="relative flex h-full min-h-[128px] w-full flex-col items-start rounded-[12px] border border-[rgba(16,185,129,0.5)] bg-white/80 px-[14px] py-[13px] text-left">
      <p className="mb-2 text-[9px] leading-[13px] font-bold tracking-[0.72px] text-[#10b981] uppercase">
        Corrective actions
      </p>
      <div className="flex w-full flex-1 flex-col gap-2">
        {actions.map((action, index) => (
          <div key={`${action}-${index}`} className="group flex items-start gap-2">
            <span className="mt-px flex size-[17px] shrink-0 items-center justify-center rounded-[5px] text-[#10b981]">
              <Icon icon="mdi:check" className="size-[11px]" aria-hidden="true" />
            </span>
            <p className="min-w-0 flex-1 text-[11.8px] leading-[17.4px] font-normal text-[#2a3446]">
              {action}
            </p>
            <button
              type="button"
              onClick={() => onRemove(index)}
              className="text-ehs-muted-text inline-flex size-4 shrink-0 items-center justify-center rounded opacity-0 transition-opacity group-hover:opacity-100 hover:text-[#e11d48]"
        ...
        <button
          type="button"
          onClick={onAdd}
          className="inline-flex items-center gap-1.5 self-start text-[11px] font-bold text-[#10b981] transition-colors hover:text-[#059669]"
        >
