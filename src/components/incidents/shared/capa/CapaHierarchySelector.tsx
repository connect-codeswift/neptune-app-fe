...
/**
 * Figma geometry 7123:41589 / Group 608 (391.69 × 335.285).
 *
 * Band colours follow the NIOSH poster ramp from the design screenshot
 * (blue → green → amber → orange → red). The raw Figma export paints every
 * band `#0891A6`, which is wrong for this control.
 *
 * Shapes are inlined rather than loaded from SVG assets because those files
 * fill with a single teal and a CSS variable can't cross an `<img>` boundary.
 *
 * Geometry is numeric so each label box is derived from its own band.
 */
type Level = Readonly<{
  id: ControlLevel;
  label: string;
   ...
   *
   * `fill` is a literal poster colour and does not follow the theme, so the
   * label cannot use a token that flips: `ehs-canvas-dark` and
   * `ehs-light-text` hold the same value in both themes.
   */
  labelClass: string;
  viewBox: string;
  path: string;
...
/**
 * NIOSH hierarchy colours (screenshot / poster), not the all-teal Figma export
 * at 7123:41589 — that fill is `#0891A6` on every band and loses the ramp.
 * Geometry still matches Figma Group 608 (391.69 × 335.285).
 */
const LEVELS: readonly Level[] = [
  {
    ...
    label: "Elimination",
    fill: "#3a72ad",
    labelClass: "text-ehs-light-text",
  ...
  },
  {
    ...
    label: "Substitution",
    fill: "#7fb539",
    labelClass: "text-ehs-canvas-dark",
...
function LevelLabel(props: Readonly<{ level: Level }>) {
  const { level } = props;

  if (level.id === "Administrative controls") {
    return (
      <>
        <span className="block leading-tight">Administrative</span>
        <span className="block leading-tight">controls</span>
      </>
    );
  }
...
export function CapaHierarchySelector(
  ...
  return (
    <div
      className={["relative mx-auto h-[335px] w-full max-w-[392px]", className]
        .filter(Boolean)
        .join(" ")}
      role="radiogroup"
    ...
    >
      {LEVELS.map((level, index) => {
        ...
        // Full poster colour until a pick is made; then dim the rest.
        const bandOpacity =
        ...
        return (
          <button
            ...
            className={[
              ...
              bandOpacity,
              isSelected ? "drop-shadow-[0_6px_14px_rgba(11,19,32,0.28)]" : "",
            ].join(" ")}
            style={{
      ...
      {LEVELS.map((level) => {
        ...
        return (
          <span
            key={`${level.id}-label`}
            className={[
              "pointer-events-none absolute z-30 flex flex-col items-center justify-center",
              "text-center text-base font-semibold tracking-[0.0917px]",
              level.labelClass,
              !hasSelection || isSelected ? "opacity-100" : "opacity-70",
            ].join(" ")}
            style={
              level.isTip
                ? {
