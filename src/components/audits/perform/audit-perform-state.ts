...
/** A question counts as answered once it carries a grade or was marked N/A. */
export function isAnswered(draft: AnswerDraft | undefined): boolean {
  return draft !== undefined && (draft.severity !== null || draft.isNA);
}
 ...
 * trip. The rule is copied deliberately: N/A is a Pass, and a question with no
 * grade is Pending.
 */
export function tallyAnswers(
  ...
  items.forEach((item) => {
    ...
    if (draft?.isNA || draft?.severity === "Pass") pass += 1;
...
export function needsNote(
  ...
  if (!isAnswered(draft) || draft?.isNA) return false;
...
export function toResponsePayload(
  ...
  return {
    ...
    responseOptionId: 0,
    valueText: draft.isNA ? "N/A" : (draft.severity ?? ""),
    note: draft.note.trim(),
    isNA: draft.isNA,
    severity: draft.isNA ? null : draft.severity,
  };
}
