...
/** Statuses that lock the run — the backend refuses writes on all of them. */
...
export function AuditPerformContent(props: AuditPerformContentProps) {
  ...
  const sections = useMemo(() => orderedSections(audit?.snapshot), [audit]);
  const items = useMemo(
  ...
  const isLocked = LOCKED_STATUSES.has((audit?.status ?? "").toLowerCase());
  ...
  const highlightBlocked = useCallback((ids: readonly number[]) => {
    ...
    document
      .querySelector(`#audit-item-${String(first)}`)
  ...
  return (
    <div className="flex flex-1 flex-col gap-3.5 px-4 pb-8">
      <AuditPerformHeader
        auditId={formatRecordDisplayId("A", audit.id)}
          ...
          <ul className="flex flex-col">
            {section.items.map((item) => (
              <AuditPerformItemRow
                ...
                attachments={attachmentsForItem(attachments, item.id)}
                disabled={isLocked}
