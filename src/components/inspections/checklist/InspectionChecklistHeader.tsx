...
export type InspectionChecklistHeaderProps = Readonly<{
  inspectionId: string;
  subtitle: string;
  onViewFindings?: () => void;
}>;

export function InspectionChecklistHeader(props: InspectionChecklistHeaderProps) {
  const { inspectionId, subtitle, onViewFindings } = props;
    ...
    <div className="relative flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[rgba(15,23,42,0.08)] bg-white px-6 py-4 shadow-[0px_12px_32px_0px_rgba(15,23,42,0.14),0px_1px_2px_0px_rgba(15,23,42,0.04)] backdrop-blur-2.5 before:pointer-events-none before:absolute before:inset-0 before:rounded-2xl before:shadow-[inset_0px_1px_0px_0px_rgba(255,255,255,0.9)] before:content-['']">
      <div className="relative z-1 flex min-w-0 flex-col gap-1.5">
        <nav aria-label="Breadcrumb" className="flex items-center gap-1">
          ...
          <Chevron />
          <span className="text-ehs-gray text-sm font-medium">Checklist</span>
        </nav>
      ...
      {onViewFindings ? (
        <Button
          ...
          className="relative z-1 shrink-0 rounded-2.5 px-4 py-2.5 text-sm font-medium"
        >
          View Findings
        </Button>
      ) : null}
