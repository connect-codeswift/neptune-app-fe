...
export function LotoEquipmentToolbar(
  ...
  return (
    <div className="border-ehs-border flex flex-wrap items-center gap-2.5 rounded-[20px] border bg-white/62 px-3.5 py-2.5 shadow-[0px_12px_32px_0px_rgba(15,23,42,0.14),0px_1px_2px_0px_rgba(15,23,42,0.04)]">
      <div className="relative min-w-0 flex-1 sm:max-w-[280px]">
        <Icon
          icon="mdi:magnify"
          className="text-ehs-muted-text pointer-events-none absolute top-1/2 left-3 size-3.5 -translate-y-1/2"
          aria-hidden="true"
        />
        <input
          type="search"
          ...
          className="border-ehs-border text-ehs-darker placeholder:text-[#8892a3] focus:border-ehs-normal-blue focus:ring-ehs-normal-blue/20 w-full rounded-[10px] border bg-white/55 py-2 pr-3 pl-9 text-[13px] outline-none focus:ring-2"
        />
      </div>
        ...
        {LOTO_STATUS_FILTERS.map((filter) => {
          ...
          return (
            <button
              ...
              className={[
                "cursor-pointer rounded-lg px-3 py-[5px] text-xs whitespace-nowrap transition-colors",
                isActive
      ...
      <span className="text-[#8892a3] ml-auto shrink-0 text-xs">
        {resultLabel}
      </span>
    </div>
  );
